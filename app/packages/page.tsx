...
export default function CategoriesIndexPage() {
  ...
  return (
    <div className="relative overflow-hidden bg-[linear-gradient(180deg,#f9fcfa_0%,#f3f8f5_100%)]">
      {/* ambient decor */}
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <motion.div
          animate={{ x: [0, 20, 0], y: [0, -18, 0] }}
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-7xl px-6 py-16 md:py-24">
          ...
          >
            ...
            >
              Browse all tour packages, filter by category, and open each
              package for full itinerary, pricing, and travel details.
            </motion.p>
      ...
      <section className="relative mx-auto max-w-7xl px-6 py-10">
        <motion.div
          ...
          viewport={{ once: true }}
          transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }}
          className="rounded-[32px] border border-[#067a47]/10 bg-white/82 p-4 shadow-[0_24px_70px_rgba(4,61,36,0.08)] backdrop-blur-xl md:p-5"
        >
          <div className="grid gap-4 lg:grid-cols-[1fr_auto] lg:items-center">
            {/* search */}
            <motion.div
              whileHover={{ y: -2 }}
              className="flex items-center gap-3 rounded-[24px] border border-[#067a47]/10 bg-white px-4 py-3 shadow-sm"
            >
              <Search className="h-5 w-5 text-[#5f756b]" />
              <input
                value={q}
                onChange={(e) => setQ(e.target.value)}
                placeholder="Search by name, subtitle, locations…"
                className="w-full bg-transparent text-sm text-[#17352a] outline-none placeholder:text-[#8ca198]"
              />
              <AnimatePresence>
                {q ? (
                  <motion.button
                    ...
                    exit={{ opacity: 0, scale: 0.7, rotate: 20 }}
                    onClick={() => setQ("")}
                    className="grid h-8 w-8 place-items-center rounded-full bg-[#f3f7f4] text-[#5f756b] transition hover:bg-[#e9f6ef]"
                    aria-label="Clear search"
                  >
                    <X className="h-4 w-4" />
                  </motion.button>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <motion.div
                whileHover={{ y: -2 }}
                className="inline-flex items-center gap-2 rounded-full border border-[#067a47]/10 bg-white px-4 py-3 text-sm text-[#5f756b] shadow-sm"
              >
                <SlidersHorizontal className="h-4 w-4 text-[#067a47]" />
                <select
                  value={sort}
                  onChange={(e) => setSort(e.target.value as SortKey)}
                  className="bg-transparent text-sm text-[#17352a] outline-none"
                >
                  <option value="popular">Sort: Popular</option>
                  <option value="priceLow">Price: Low to High</option>
                  <option value="priceHigh">Price: High to Low</option>
              ...
              <motion.button
                ...
                className="rounded-full border border-[#067a47]/10 bg-white px-5 py-3 text-sm font-semibold text-[#17352a] shadow-sm transition hover:bg-[#f7faf8]"
              >
                Reset
              </motion.button>
            </div>
                ...
                >
                  {isActive ? (
                    <motion.span
                      layoutId="activeCategoryPill"
                      transition={{ type: "spring", stiffness: 300, damping: 30 }}
                      className="absolute inset-0 rounded-full bg-gradient-to-r from-[#067a47] to-[#035c35]"
                    />
                  ) : null}

                  {!isActive ? (
                    <span className="absolute inset-0 rounded-full bg-[#f7faf8] ring-1 ring-[#067a47]/10 transition hover:bg-white" />
                  ) : null}
                  ...
                  <span
                    className={`relative z-10 ${
                      isActive ? "text-white" : "text-[#17352a]"
                    }`}
                  >
                    {c.title}
                  </span>
        ...
        <AnimatePresence mode="popLayout">
          {filtered.length > 0 ? (
            <motion.div layout className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {filtered.map((p, index) => (
                <motion.article
                  ...
                  whileHover={{ y: -8 }}
                  className="group relative overflow-hidden rounded-[30px] border border-[#067a47]/10 bg-white shadow-[0_16px_40px_rgba(4,61,36,0.07)] transition hover:shadow-[0_24px_70px_rgba(4,61,36,0.14)]"
                >
                  {/* glow */}
                  <motion.div
                    animate={{ opacity: [0.15, 0.4, 0.15] }}
                    transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
                    className="pointer-events-none absolute inset-x-10 top-0 h-24 rounded-full bg-[#f0ab1d]/15 blur-3xl"
                  />
                  ...
                  <div className="relative h-48 w-full overflow-hidden">
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-[#071c14]/65 via-[#071c14]/8 to-transparent" />
                    ...
                    <div className="absolute bottom-3 left-4 rounded-full bg-white/16 px-3 py-1 text-sm font-semibold text-white backdrop-blur-sm">
                      From ${p.basePrice.toLocaleString()}
                    </div>
                    ...
                    <motion.button
                      onClick={() => router.push(`/packages/${p.id}`)}
                      whileHover={{ scale: 1.02 }}
                      className="absolute right-4 top-4 inline-flex items-center gap-2 rounded-full border border-white/40 bg-white/85 px-4 py-2 text-xs font-semibold tracking-[0.18em] text-[#17352a] shadow-sm backdrop-blur"
                    >
                  ...
                  >
                    <motion.div
                      variants={fadeUp}
                      className="text-[11px] font-bold tracking-[0.18em] text-[#b98716]"
                    >
                      PACKAGE #{p.no}
                    </motion.div>
                    ...
                    <motion.div
                      variants={fadeUp}
                      className="mt-2 text-xl font-semibold text-[#17352a]"
                      style={{ fontFamily: "var(--font-playfair, serif)" }}
                    >
                      {p.name}
                    </motion.div>
                    ...
                    <motion.div
                      variants={fadeUp}
                      className="mt-1 text-sm text-[#5f756b]"
                    >
                      {p.subtitle}
                    </motion.div>
                    ...
                    <motion.p
                      variants={fadeUp}
                      className="mt-4 line-clamp-3 text-sm leading-7 text-[#4c645a]"
                    >
                      {p.description}
                    </motion.p>
                    ...
                    <motion.div
                      variants={fadeUp}
                      className="mt-5 flex flex-wrap gap-2 text-xs text-[#17352a]"
                    >
                      <motion.span
                        whileHover={{ y: -2, scale: 1.03 }}
                        className="inline-flex items-center gap-2 rounded-full bg-[#f7faf8] px-3 py-1.5 ring-1 ring-[#067a47]/10"
                      >
                        <Clock className="h-4 w-4 text-[#067a47]" />
                        {p.duration}
                      </motion.span>
                    ...
                    <motion.button
                      ...
                      whileTap={{ scale: 0.985 }}
                      onClick={() => router.push(`/packages/${p.id}`)}
                      className="relative mt-6 inline-flex w-full items-center justify-center overflow-hidden rounded-full bg-gradient-to-r from-[#067a47] to-[#035c35] px-5 py-3 text-sm font-semibold text-white shadow-[0_12px_30px_rgba(6,122,71,0.22)]"
                    >
                      <motion.span
                        animate={{ x: ["-120%", "120%"] }}
                        transition={{ duration: 2.2, repeat: Infinity, ease: "linear" }}
            ...
            <motion.div
              ...
              animate={{ opacity: 1, y: 0, scale: 1 }}
              exit={{ opacity: 0, y: 10 }}
              className="mt-12 rounded-[30px] border border-[#067a47]/10 bg-white p-10 text-center shadow-[0_16px_40px_rgba(4,61,36,0.06)]"
            >
              <div
              ...
              <div className="mt-2 text-[#5f756b]">
                Try a different keyword or switch categories.
              </div>
            </motion.div>
          )}
